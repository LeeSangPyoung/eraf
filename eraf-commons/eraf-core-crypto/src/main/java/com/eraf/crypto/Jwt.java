...
public final class Jwt {
     ...
     * @return JWT 토큰
     */
    public static String createWithRSA(Map<String, Object> claims, PrivateKey privateKey, long expirationMinutes) {
        Instant now = Instant.now();
    ...
    /**
     * JWT 생성 (RS256, 기본 1시간 만료)
     */
    public static String createWithRSA(Map<String, Object> claims, PrivateKey privateKey) {
        return createWithRSA(claims, privateKey, 60);
    }

    /**
     * JWT 검증 및 클레임 추출 (RS256)
     *
     * @param token     JWT 토큰
     * @param publicKey RSA 공개 키
     * @return 클레임
     */
    public static Claims verifyWithRSA(String token, PublicKey publicKey) {
        try {
            return Jwts.parser()
    ...
    /**
     * JWT 검증 (RS256, 예외 없이 Optional 반환)
     */
    public static Optional<Claims> verifyWithRSAQuietly(String token, PublicKey publicKey) {
        try {
            return Optional.of(verifyWithRSA(token, publicKey));
        } catch (Exception e) {
            return Optional.empty();
        }
    }
     ...
     * RSA 키 쌍 생성 (2048 bit)
     *
     * @return RSA KeyPair (PrivateKey + PublicKey)
     */
     ...
     * RSA 키 쌍 생성
     *
     * @param keySize 키 크기 (2048 또는 4096 권장)
     * @return RSA KeyPair
     */
    public static KeyPair generateRSAKeyPair(int keySize) {
        try {
            ...
            throw new CryptoException("RSA 키 쌍 생성 실패", e);
        }
    }

    /**
     * Base64 인코딩된 문자열에서 RSA PrivateKey 복원
     */
    public static PrivateKey decodePrivateKey(String base64EncodedKey) {
        try {
            ...
            throw new CryptoException("RSA 개인 키 복원 실패", e);
        }
    }

    /**
     * Base64 인코딩된 문자열에서 RSA PublicKey 복원
     */
    public static PublicKey decodePublicKey(String base64EncodedKey) {
        try {
            byte[] keyBytes = Base64.getDecoder().decode(base64EncodedKey);
            ...
            throw new CryptoException("RSA 공개 키 복원 실패", e);
        }
    }

    /**
     * RSA 키를 Base64 문자열로 인코딩
     */
